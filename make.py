...
def kfp(data, row, col):
	if(type(data) != list):
		print("kfp gets sketchy on non lists...Always use numeric indicies...attempting -_- ?")
		try:
			data = data.toArray()
		except AttributeError:
			data = [[k[k2] for k2 in k]for k in data]		
	return(fp.fetchpPrediction(data, row, col))
